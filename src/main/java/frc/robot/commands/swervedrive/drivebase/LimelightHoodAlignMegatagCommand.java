...
public class LimelightHoodAlignMegatagCommand extends Command {
    ...
    public LimelightHoodAlignMegatagCommand(SwerveSubsystem swerve, ShooterSubsystem shooter) {
    ...
    private void updatePositioningState() {
        ...
        if (allience == Alliance.Red) {
            targetX = 9.83;
            distanceFromHubAprilTag = -0.55;
        } else if (allience == Alliance.Blue) {
            targetX = 4.52;
            distanceFromHubAprilTag = 0.55;
        }

        double distanceToTarget = Math.sqrt(Math.pow(botPoseX - targetX + distanceFromHubAprilTag, 2) + Math.pow(botPoseY - 4.02, 2));
